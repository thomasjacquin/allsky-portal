...
function DisplayLiveView($image_name){
  ...
  if ($d == '')
  {
	...
	// during the day so just use the delay.
	$daydelay = $camera_settings_array["daydelay"];

	// The time between night exposures is (nightdelay + nightmaxexposure).  Both can be large numbers
	// so use both.
	$nightdelay = ($camera_settings_array["nightdelay"] + $camera_settings_array['nightmaxexposure']);
